...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res, options) => {
    const {
      isTransformResponse, isReturnNativeResponse
    } = options;
  ...
  beforeRequestHook: (config, options) => {
    const {
      apiUrl, isJoinPrefix, urlPrefix, joinParamsToUrl, formatDate, joinTime = true
    } = options;
  ...
  requestInterceptors: (config, options) => {
    ...
    if (token && (config as TRecordable)?.requestOptions?.withToken !== false) {

      // jwt token
      (config as TRecordable).headers.Authorization = options.authenticationScheme
  ...
  responseInterceptorsCatch: (error: any, instance: AxiosInstance) => {
    ...
    config.headers = {
      ...config.headers,
      "Content-Type": EContentTypeEnum.Json
    };
