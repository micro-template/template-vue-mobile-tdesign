...
import {
  ICreateAxiosOptions as CreateAxiosOptions
} from "./AxiosTransform";
...
import {
  IAxiosRequestConfigRetry as AxiosRequestConfigRetry,
...
export class VAxios {
  ...
  supportFormData(config: AxiosRequestConfig) {
    ...
    if (
      contentType !== EContentTypeEnum.FormURLEncoded ||
  ...
  supportParamsStringify(config: AxiosRequestConfig) {
    ...
    if (contentType === EContentTypeEnum.FormURLEncoded || !Reflect.has(config, "params")) {
      return config;
    }

    return {
      ...config,
      paramsSerializer: (params: unknown) => stringify(params, {
        arrayFormat: "brackets"
      })
    };
  }
    ...
    return this.request(
        {
          ...
          method: "POST",
          headers: {
            "Content-Type": EContentTypeEnum.FormData
          },
          params
        },
        options
  ...
  private async synthesisRequest<T = any>(config: AxiosRequestConfigRetry, options?: RequestOptions): Promise<T> {
    ...
    const {
      beforeRequestHook, requestCatchHook, transformRequestHook
    } = transform || {};
